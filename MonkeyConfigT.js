...
function MonkeyConfig(data) {
  ...
  // Fungsi untuk memuat DOMPurify secara dinamis
  function loadDOMPurify() {
    return new Promise((resolve, reject) => {
      ...
      script.integrity = 'sha384-qSFej5dZNviyoPgYJ5+Xk4bEbX8AYddxAHPuzs1aSgRiXxJ3qmyWNaPsRkpv/+x5';
  ...
  let trustedPolicy;
  async function initTrustedPolicy() {
    try {
      await loadDOMPurify();
      ...
      log(`Failed to load DOMPurify or create Trusted Types policy: ${e.message}`, 'error');
      trustedPolicy = {
  ...
  // Panggil initTrustedPolicy sebelum init lainnya
  initTrustedPolicy().then(() => {
    init();
  }).catch(e => {
    log(`Failed to initialize Trusted Types: ${e.message}`, 'error');
    init(); // Lanjutkan dengan fallback
  });
  function init() {
    try {
    ...
    try {
      ...
      for (let key in params) {
        ...
        if (param.type === 'checkbox') {
          ...
          try {
            values[key] = param.get(root.querySelector(`#__MonkeyConfig_parent_${MonkeyConfig.esc(key)}`));
          } catch (e) {
