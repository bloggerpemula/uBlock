 ...
 * Fixed: Limited dialog height and recalculated position - March 2025
 * Fixed: Ensured overlay covers full viewport - March 2025
 */
function MonkeyConfig(data) {
    ...
    function forcePositionAndSize() {
        ...
        var dialogHeight = Math.min(layer.getBoundingClientRect().height, viewportHeight * 0.8); // Batasi 80% viewport

        // Paksa posisi di tengah viewport berdasarkan tinggi aktual
        ...
        layer.style.maxHeight = `${viewportHeight * 0.8}px`; // Batasi tinggi
        layer.style.overflowY = 'auto'; // Scroll jika terlalu tinggi
        layer.style.zIndex = '10000';

        console.log('MonkeyConfig: Forced position - top:', layer.style.top, 'left:', layer.style.left, 'width:', layer.style.width, 'max-height:', layer.style.maxHeight);
    }
    ...
    function open() {
        ...
        function openDone() {
            ...
            if (layer && overlay && container) {
                forcePositionAndSize(); // Paksa posisi dan batasi tinggi
                if (data.adjustOverlaySize) { 
                    adjustOverlaySize(); 
                    console.log('MonkeyConfig: Adjusted overlay size');
                } else { 
                    defaultOverlaySize(); // Overlay penuh
        ...
        try {
            ...
            setTimeout(openDone, 0); // Jalankan setelah DOM diperbarui
        } catch (e) {
