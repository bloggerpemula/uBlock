// ==UserScript==
    ...
    // @description     Enhanced Configuration Dialog Builder with column layout, custom styling, scrollable labels, and customizable checkbox/number sizes
    // ==/UserScript==
    /*
     ...
     * Additions: Scrollable labels on small width/height, customizable checkbox/number sizes with manual width/height and corner alignment options
     */
    function MonkeyConfig(data) {
        ...
        function update() {
            ...
            for (let key in params) {
                ...
                if (param.type === 'checkbox') {
                    ...
                    applyCornerAlignment(elem, param);
        ...
        function applyCornerAlignment(elem, param) {
            if (param.leftTop) {
                elem.style.position = 'absolute';
                elem.style.left = '0';
                elem.style.top = '0';
            } else if (param.rightTop) {
                elem.style.position = 'absolute';
                elem.style.right = '0';
                elem.style.top = '0';
            } else if (param.leftBottom) {
                elem.style.position = 'absolute';
                elem.style.left = '0';
                elem.style.bottom = '0';
            } else if (param.rightBottom) {
                elem.style.position = 'absolute';
                elem.style.right = '0';
                elem.style.bottom = '0';
            }
    ...
    MonkeyConfig.HTML = {
        ...
        _label: (name, opt) => `<label for="__MonkeyConfig_field_${name}"${opt.labelAlign || opt.fontSize || opt.fontColor ? ` style="${[opt.labelAlign && `text-align:${opt.labelAlign}`, opt.fontSize && `font-size:${opt.fontSize}`, opt.fontColor && `color:${opt.fontColor}`].filter(Boolean).join(';')};overflow-x:auto;white-space:nowrap;"` : ' style="overflow-x:auto;white-space:nowrap;"'}>${opt.label || name.charAt(0).toUpperCase() + name.slice(1).replace(/_/g, ' ')}</label>`,
        checkbox: (name, opt) => `<input id="__MonkeyConfig_field_${name}" type="checkbox" name="${name}" style="width:${opt.width || '11px'};height:${opt.height || '11px'};${opt.leftTop ? 'position:absolute;left:0;top:0;' : ''}${opt.rightTop ? 'position:absolute;right:0;top:0;' : ''}${opt.leftBottom ? 'position:absolute;left:0;bottom:0;' : ''}${opt.rightBottom ? 'position:absolute;right:0;bottom:0;' : ''}" />`,
        custom: (name, opt) => opt.html || '',
        number: (name, opt) => `<input id="__MonkeyConfig_field_${name}" type="number" class="__MonkeyConfig_field_number" name="${name}" min="${opt.min || ''}" max="${opt.max || ''}" step="${opt.step || '1'}" style="width:${opt.width || '40px'};height:${opt.height || '20px'};${opt.leftTop ? 'position:absolute;left:0;top:0;' : ''}${opt.rightTop ? 'position:absolute;right:0;top:0;' : ''}${opt.leftBottom ? 'position:absolute;left:0;bottom:0;' : ''}${opt.rightBottom ? 'position:absolute;right:0;bottom:0;' : ''}" />`,
...
.__MonkeyConfig_container td { border: none !important; line-height: 100% !important; padding: 0.3em !important; text-align: left !important; vertical-align: middle !important; white-space: normal !important; position: relative !important; }
...
@media (max-width: 600px) { .__MonkeyConfig_columns { flex-direction: column !important; } .__MonkeyConfig_left_column, .__MonkeyConfig_right_column { width: 100% !important; } }`
    ...
    const config = new MonkeyConfig({
        ...
        shadowWidth: '300px', // Small width to trigger label scrolling
        shadowHeight: '200px', // Small height to trigger label scrolling
        menuCommand: true,
        parameters: {
            enable_feature: {
                type: 'checkbox',
                label: 'Enable Feature with Long Label Text That Should Scroll Horizontally',
                default: false,
                width: '20px', // Custom checkbox width
                height: '20px', // Custom checkbox height
                leftTop: true // Align to left-top corner
            },
            volume_level: {
                type: 'number',
                label: 'Volume Level with Scrollable Long Label Text',
                default: 50,
                min: 0,
                max: 100,
                width: '60px', // Custom number width
                height: '30px', // Custom number height
                rightBottom: true // Align to right-bottom corner
            },
            user_name: {
                type: 'text',
                label: 'User Name',
