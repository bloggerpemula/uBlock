...
// @description     Enhanced Configuration Dialog Builder with column layout, custom styling, additional input types, scrollable labels, and customizable input sizes
// ==/UserScript==
/*
 ...
 * Further Enhanced - April 2025
 * Additions: Scrollable labels on small dimensions, customizable checkbox/number sizes, corner positioning (left&top, right&top, left&bottom, right&bottom)
...
function MonkeyConfig(data) {
    ...
    function init() {
        ...
        cfg.position = data.position || storedValues.position || "center"; // New: corner positioning
        cfg.checkboxWidth = data.checkboxWidth || storedValues.checkboxWidth || "11px"; // New: checkbox width
        cfg.checkboxHeight = data.checkboxHeight || storedValues.checkboxHeight || "11px"; // New: checkbox height
        cfg.numberWidth = data.numberWidth || storedValues.numberWidth || "40px"; // New: number width
        cfg.numberHeight = data.numberHeight || storedValues.numberHeight || "20px"; // New: number height
    ...
    function update() {
        ...
        for (let key in params) {
            ...
            if (param.type === 'checkbox') {
                elem.style.width = param.checkboxWidth || cfg.checkboxWidth;
                elem.style.height = param.checkboxHeight || cfg.checkboxHeight;
                elem.style.backgroundColor = 'inherit';
                elem.style.color = labelFontColor;}
            else if (param.type === 'number') {
                elem.style.width = param.numberWidth || cfg.numberWidth;
                elem.style.height = param.numberHeight || cfg.numberHeight;}
    ...
    function saveClick() {
        ...
        const allValues = {
            ...
            checkboxWidth: cfg.checkboxWidth,
            checkboxHeight: cfg.checkboxHeight,
            numberWidth: cfg.numberWidth,
    ...
    function open() {
        ...
        let positionStyle = 'top: 50% !important; left: 50% !important; transform: translate(-50%, -50%) !important;';
        switch (cfg.position) {
            case 'left_top':
        ...
        if (!container || shadowRoot.querySelector('.__MonkeyConfig_overlay').offsetHeight === 0) {
            ...
            let iframePositionStyle = 'top: 50% !important; left: 50% !important; transform: translate(-50%, -50%) !important;';
            switch (cfg.position) {
                case 'left_top':
            ...
            iframeDoc.write(`<!DOCTYPE html><html><head><style>
                ...
                .__MonkeyConfig_container td.__MonkeyConfig_inline input[type="checkbox"] { width: ${cfg.checkboxWidth} !important; height: ${cfg.checkboxHeight} !important; margin: 0 0.5em 0 0 !important; vertical-align: middle !important; accent-color: #007bff !important; display: inline-block !important; }
                .__MonkeyConfig_container td.__MonkeyConfig_inline input[type="number"] { width: ${cfg.numberWidth} !important; height: ${cfg.numberHeight} !important; margin: 0 0.5em 0 0 !important; vertical-align: middle !important; display: inline-block !important; }
...
MonkeyConfig.HTML = {
    ...
    checkbox: (name, opt) => `<input id="__MonkeyConfig_field_${name}" type="checkbox" name="${name}" style="width:${opt.checkboxWidth || cfg.checkboxWidth};height:${opt.checkboxHeight || cfg.checkboxHeight};" />`,
    custom: (name, opt) => opt.html || '',
    number: (name, opt) => `<input id="__MonkeyConfig_field_${name}" type="number" class="__MonkeyConfig_field_number" name="${name}" min="${opt.min || ''}" max="${opt.max || ''}" step="${opt.step || '1'}" style="width:${opt.numberWidth || cfg.numberWidth};height:${opt.numberHeight || cfg.numberHeight};" />`,
