 ...
 * v2.6 Updates - March 2025:
 * - Dynamic iframe sizing based on content
 * - Overlay moved to main DOM for full coverage
 * - Strengthened h1 styling in iframe
 * - Fixed icon loading
 */
function MonkeyConfig(data) {
    ...
    function log(message) {
        // Hapus filter kondisional, cetak semua log
    ...
    function init() {
        ...
        cfg.iframeHeight = data.iframeHeight || storedValues.iframeHeight || "400px"; // Sesuaikan default iframeHeight agar lebih besar
        ...
        log(`Initialized with title: ${cfg.title}, shadowWidth: ${cfg.shadowWidth}, shadowHeight: ${cfg.shadowHeight}, iframeWidth: ${cfg.iframeWidth}, iframeHeight: ${cfg.iframeHeight}`);
    ...
    function open() {
        ...
        const shadowHeight = cfg.shadowHeight || "400px";
        ...
        if (!container || shadowRoot.querySelector('.__MonkeyConfig_overlay').offsetHeight === 0) {
            ...
            const iframeHeight = cfg.iframeHeight || "400px";
            log(`Switching to iframe with dimensions - Width: ${iframeWidth}, Height: ${iframeHeight}`);

            iframeFallback.style.cssText = `position: fixed !important; top: 50% !important; left: 50% !important; transform: translate(-50%, -50%) !important; width: ${iframeWidth} !important; height: ${iframeHeight} !important; max-width: 90vw !important; max-height: 80vh !important; z-index: 2147483647 !important; border: none !important; background: #eee !important;`;
            body.appendChild(iframeFallback);
            const iframeDoc = iframeFallback.contentDocument;
            iframeDoc.open();
            iframeDoc.write(`<!DOCTYPE html><html><head><style>
                body { margin: 0 !important; padding: 0 !important; background: #eee linear-gradient(180deg, #f8f8f8 0, #ddd 100%) !important; font-family: Arial, sans-serif !important; font-size: ${cfg.iframeFontSize} !important; color: ${cfg.iframeFontColor} !important; }
                ...
                .__MonkeyConfig_overlay { position: fixed !important; top: 0 !important; left: 0 !important; width: 100vw !important; height: 100vh !important; background-color: rgba(0, 0, 0, 0.6) !important; z-index: 2147483646 !important; }
                .__MonkeyConfig_container { position: relative !important; width: 100% !important; height: 100% !important; padding: 1em !important; box-sizing: border-box !important; overflow-y: auto !important; }
            </style></head><body><div class="__MonkeyConfig_overlay"></div>${render()}</body></html>`);
            iframeDoc.close();
            openLayer = iframeFallback;
            openDone(iframeDoc);

            // Log dimensi aktual iframe
