...
function MonkeyConfig(data) {
  ...
  // Logger dengan penanganan error
  ...
  function init() {
    try {
      ...
      let storedValues;
      try {
        storedValues = GM_getValue(storageKey) ? JSON.parse(GM_getValue(storageKey)) : {};
      } catch (e) {
      ...
      if (data.menuCommand) {
        try {
          GM_registerMenuCommand(data.menuCommand === true ? cfg.title : data.menuCommand, () => cfg.open());
        } catch (e) {
  ...
  function render() {
    try {
      ...
      throw new Error('Failed to render configuration dialog');
    }
  }
    ...
    try {
      ...
      for (let key in params) {
        ...
        if (param.type === 'checkbox') {
          ...
          try {
            param.set(values[key], root.querySelector(`#__MonkeyConfig_parent_${MonkeyConfig.esc(key)}`));
          } catch (e) {
    ...
    try {
      ...
      for (let key in params) {
        ...
        if (param.type === 'checkbox') {
          ...
          try {
            value = param.get(root.querySelector(`#__MonkeyConfig_parent_${MonkeyConfig.esc(key)}`));
          } catch (e) {
          ...
          value = root.querySelector(`[name="${MonkeyConfig.esc(key)}"]:checked`)?.value || '';
        } else if (param.type === 'file') {
          value = elem.dataset.value || values[key];
        } else if (param.type === 'select') {
          if (elem.type === 'checkbox') {
            value = Array.from(root.querySelectorAll(`input[name="${MonkeyConfig.esc(key)}"]:checked`)).map(input => input.value);
          } else if (elem.multiple) {
            value = Array.from(root.querySelectorAll(`select[name="${MonkeyConfig.esc(key)}"] option:selected`)).map(opt => opt.value);
          } else {
            value = elem.value;
          }
        }
  ...
  function open() {
    ...
    if (!shadowRoot) {
      ...
      try {
        iframeDoc.open();
        const iframeHTML = createTrustedHTML(`
          <!DOCTYPE html>
          <html>
          <head>
...
MonkeyConfig.esc = function esc(string) {
  try {
    ...
    MonkeyConfig.log(`Failed to escape string: ${e.message}`, 'error');
    return '';
  }
};
...
MonkeyConfig.HTML = {
  _field: function _field(name, opt) {
    try {
      const field = opt.type && MonkeyConfig.HTML[opt.type] ? (opt.html ? opt.html.replace(/\[FIELD\]/, MonkeyConfig.HTML[opt.type](name, opt)) :
        MonkeyConfig.HTML[opt.type](name, opt)) : '';
      return MonkeyConfig.createTrustedHTML(field);
    } catch (e) {
      MonkeyConfig.log(`Failed to render field ${name}: ${e.message}`, 'error');
      return MonkeyConfig.createTrustedHTML('');
    }
  },
  _label: function _label(name, opt) {
    try {
      const html = `<label for="__MonkeyConfig_field_${MonkeyConfig.esc(name)}"${
      ...
      }>${MonkeyConfig.esc(opt.label || name.charAt(0).toUpperCase() + name.slice(1).replace(/_/g, ' '))}</label>`;
      return MonkeyConfig.createTrustedHTML(html);
    } catch (e) {
      MonkeyConfig.log(`Failed to render label for ${name}: ${e.message}`, 'error');
      return MonkeyConfig.createTrustedHTML('');
    }
  },
  checkbox: function checkbox(name) {
    return MonkeyConfig.createTrustedHTML(`<input id="__MonkeyConfig_field_${MonkeyConfig.esc(name)}" type="checkbox" name="${MonkeyConfig.esc(name)}" />`);
  },
  custom: function custom(name, opt) {
    return MonkeyConfig.createTrustedHTML(opt.html || '');
  },
  number: function number(name, opt) {
    return MonkeyConfig.createTrustedHTML(`<input id="__MonkeyConfig_field_${MonkeyConfig.esc(name)}" type="number" class="__MonkeyConfig_field_number" name="${MonkeyConfig.esc(name)}" min="${MonkeyConfig.esc(opt.min || '')}" max="${MonkeyConfig.esc(opt.max || '')}" step="${MonkeyConfig.esc(opt.step || '1')}" />`);
  },
  text: function text(name) {
    return MonkeyConfig.createTrustedHTML(`<input id="__MonkeyConfig_field_${MonkeyConfig.esc(name)}" type="text" class="__MonkeyConfig_field_text" name="${MonkeyConfig.esc(name)}" />`);
  },
  color: function color(name) {
    return MonkeyConfig.createTrustedHTML(`<input id="__MonkeyConfig_field_${MonkeyConfig.esc(name)}" type="color" class="__MonkeyConfig_field_text" name="${MonkeyConfig.esc(name)}" />`);
  },
  textarea: function textarea(name, opt) {
    return MonkeyConfig.createTrustedHTML(`<textarea id="__MonkeyConfig_field_${MonkeyConfig.esc(name)}" class="__MonkeyConfig_field_text" name="${MonkeyConfig.esc(name)}" rows="${MonkeyConfig.esc(opt.rows || 4)}" cols="${MonkeyConfig.esc(opt.cols || 20)}"></textarea>`);
  },
  range: function range(name, opt) {
    return MonkeyConfig.createTrustedHTML(`<input id="__MonkeyConfig_field_${MonkeyConfig.esc(name)}" type="range" name="${MonkeyConfig.esc(name)}" min="${MonkeyConfig.esc(opt.min || 0)}" max="${MonkeyConfig.esc(opt.max || 100)}" step="${MonkeyConfig.esc(opt.step || 1)}" />`);
  },
  radio: function radio(name, opt) {
    try {
      const html = Object.entries(opt.choices).map(([val, text]) =>
        `<label><input type="radio" name="${MonkeyConfig.esc(name)}" value="${MonkeyConfig.esc(val)}" /> ${MonkeyConfig.esc(text)}</label><br/>`
      ).join('');
      return MonkeyConfig.createTrustedHTML(html);
    } catch (e) {
      MonkeyConfig.log(`Failed to render radio for ${name}: ${e.message}`, 'error');
      return MonkeyConfig.createTrustedHTML('');
    }
  },
  file: function file(name, opt) {
    return MonkeyConfig.createTrustedHTML(`<input id="__MonkeyConfig_field_${MonkeyConfig.esc(name)}" type="file" name="${MonkeyConfig.esc(name)}" accept="${MonkeyConfig.esc(opt.accept || '*/*')}" />`);
  },
  ...
  group: function group(name, opt) {
    try {
      ...
      MonkeyConfig.log(`Failed to render group for ${name}: ${e.message}`, 'error');
      return MonkeyConfig.createTrustedHTML('');
    }
  },
  select: function select(name, opt) {
    try {
      const choices = Array.isArray(opt.choices) ? Object.fromEntries(opt.choices.map(val => [val, val])) :
        opt.choices;
      const html = `<select id="__MonkeyConfig_field_${MonkeyConfig.esc(name)}" class="__MonkeyConfig_field_select" name="${MonkeyConfig.esc(name)}"${
      ...
      MonkeyConfig.log(`Failed to render select for ${name}: ${e.message}`, 'error');
      return MonkeyConfig.createTrustedHTML('');
    }
  }
};
...
MonkeyConfig.formatters = {
  tr: function tr(name, opt) {
    try {
      ...
      MonkeyConfig.log(`Failed to format table row for ${name}: ${e.message}`, 'error');
      return MonkeyConfig.createTrustedHTML('');
    }
  }
};
