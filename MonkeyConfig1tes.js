...
function MonkeyConfig(data) {
  ...
  function render() {
   let html = `<div class="__MonkeyConfig_container">
    ...
    <h1>${cfg.title}</h1><div class="__MonkeyConfig_content"><div class="__MonkeyConfig_top">`;
    for (let key in params)
  ...
  function open() {
    ...
    async function fetchNopechaStatus(apiKey = '') {
        const host_api = 'https://api.nopecha.com';
        try {
            const headers = {
            ...
            if (!response.error) {
                const message = `NopeCHA Anda: ${response.status} Dengan Plan ${response.plan} Dan ${response.quota} Quota, Masih Memiliki ${response.credit} Credit`;
                GM_setValue('nopecha_status', message);
                return message;
            } else {
                const errorMessage = `NopeCHA Error: ${response.error}`;
                GM_setValue('nopecha_status', errorMessage);
                return errorMessage;
            }
        } catch (e) {
            const errorMessage = `Failed to fetch NopeCHA status: ${e.message}`;
            GM_setValue('nopecha_status', errorMessage);
            return errorMessage;
        }
    }
    if (typeof MonkeyConfig !== 'undefined') {
        MonkeyConfig.fetchNopechaStatus = fetchNopechaStatus;
        const originalInit = MonkeyConfig.prototype.init || function() {};
        MonkeyConfig.prototype.init = function() {
            originalInit.apply(this, arguments);
            const apiKey = this.get('NopeCHA') || ''; // Ambil kunci API dari pengaturan
            MonkeyConfig.fetchNopechaStatus(apiKey).then((message) => {
                if (typeof BpNote !== 'undefined') {
                    BpNote(message, 'info');
                } else {
                    console.log(`[MonkeyConfig] ${message}`);
                }
            });
        };
    } else {
        window.fetchNopechaStatus = fetchNopechaStatus;
    }
  function close() {
