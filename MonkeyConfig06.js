...
function MonkeyConfig(data) {
    ...
    cfg.updateDimensions = function() {
        const values = cfg.values || {};
        const width = (values.ShadowWidth || 600) + 'px';
        const height = (values.ShadowHeight || 350) + 'px';
        const iframeW = (values.IframeWidth || 600) + 'px';
        const iframeH = (values.IframeHeight || 300) + 'px';
        // Update properti instance
        cfg.shadowWidth = width;
        cfg.shadowHeight = height;
        cfg.iframeWidth = iframeW;
        cfg.iframeHeight = iframeH;
        // Apply ke elemen yang sedang aktif
        if (shadowRoot) {
            const container = shadowRoot.querySelector('.__MonkeyConfig_container');
            if (container) {
                container.style.width = width;
                container.style.height = height;}
        } else if (iframeFallback) {
            iframeFallback.style.width = iframeW;
            iframeFallback.style.height = iframeH;
        }};
    ...
    function open() {
        ...
        if (!container || shadowRoot.querySelector('.__MonkeyConfig_overlay').offsetHeight === 0) {
            ...
            iframeDoc.write(`<!DOCTYPE html><html><head><style>
                ...
                .__MonkeyConfig_container td.__MonkeyConfig_inline input[type="checkbox"] { width: 32px !important; height: 32px !important; margin: 0 0.5em 0 0 !important; vertical-align: middle !important; accent-color: #007bff !important; display: inline-block !important; }
